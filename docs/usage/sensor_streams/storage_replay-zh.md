...
以 pickle 文件格式存储带时间戳的传感器数据。每帧保存为 `000.pickle`、`001.pickle` 等。
...
**存储内容：** 默认情况下，如果帧有 `.raw_msg` 属性，则序列化该属性而非完整对象。你可以通过 `autocast` 参数自定义：
...
一种常见模式是创建基于回放的连接桩，用于在没有硬件的情况下进行测试。来自 [`robot/unitree/go2/connection.py`](/dimos/robot/unitree/go2/connection.py#L83)：

这种方式还比较原始。我们希望编写一个更高层的 API 来录制任意模块的完整 I/O，但目前仍在开发中。
...
- **data**：传感器数据（如果提供了 `autocast` 则为其返回值）

文件按顺序编号：`000.pickle`、`001.pickle` 等。

录制数据存储在 `data/` 目录中。参阅[数据加载](/docs/development/large_file_management.md)了解数据存储的工作方式，包括大型数据集的 Git LFS 处理。
...
| `save(*frames)`              | 保存多帧                          |
| `save_stream(observable)`    | 将 observable 通过存储管道传递     |
| `consume_stream(observable)` | 订阅并保存，不返回值               |

### TimedSensorReplay

| 方法                                              | 描述                           |
|--------------------------------------------------|-------------------------------|
| `iterate(loop=False)`                            | 迭代帧（不带时序）              |
| `iterate_ts(seek, duration, loop)`               | 带绝对时间戳迭代                |
| `iterate_duration(...)`                          | 带相对时间戳迭代                |
| `iterate_realtime(speed, ...)`                   | 阻塞式迭代以匹配时序             |
