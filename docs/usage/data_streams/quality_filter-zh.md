...
在处理传感器流时，你经常需要降低频率同时保留最高质量的数据。对于图像这类无法平均或合并的离散数据，`quality_barrier` 不是盲目丢弃帧，而是在每个时间窗口内选择质量最高的项。

## 问题

摄像头输出 30fps，但你的 ML 模型只需要 2fps。简单的方法：

- **`sample(0.5)`** - 取恰好落在间隔时间点的帧
- **`throttle_first(0.5)`** - 取第一帧，忽略其余的

两者都忽略了质量。你可能得到一个模糊的帧，而清晰的帧明明是可用的。
...
data = [
    {"id": 1, "quality": 0.3},
    {"id": 2, "quality": 0.9},  # 第一个窗口中最好的
    {"id": 3, "quality": 0.5},
    {"id": 4, "quality": 0.2},
    {"id": 5, "quality": 0.8},  # 第二个窗口中最好的
    {"id": 6, "quality": 0.4},
]

source = rx.of(*data)

# 每个窗口选择最高质量的项（每秒 2 项 = 0.5 秒窗口）
result = source.pipe(
...
对于摄像头数据流，我们提供了 `sharpness_barrier`，它使用图像的清晰度分数。

让我们使用 Unitree Go2 机器人的真实摄像头数据进行演示。我们使用[传感器存储与回放](/docs/usage/sensor_streams/storage_replay.md)工具包，该工具包提供对已录制机器人数据的访问：
...
# 使用 stream() 并设置 seek 跳过空白帧，speed=10x 加速采集
...
清晰度分数（0.0 到 1.0）使用 Sobel 边缘检测计算：

出自 [`Image.py`](/dimos/msgs/sensor_msgs/Image.py)
...
你可以将 `quality_barrier` 与任何质量指标配合使用：

```python session=qb
# 示例：按 "confidence" 字段选择
detections = [
    {"name": "cat", "confidence": 0.7},
    {"name": "dog", "confidence": 0.95},  # 最好的
...
| `target_frequency` | `float` | 输出频率，单位 Hz（例如 2.0 表示每秒 2 个项） |

**返回值：** 用于 `.pipe()` 的管道操作符

### `sharpness_barrier(target_frequency)`

针对图像的便捷包装器，使用 `image.sharpness` 作为质量函数。

| 参数 | 类型 | 描述 |
|--------------------|---------|--------------------------|
| `target_frequency` | `float` | 输出频率，单位 Hz |
